...
data <- data  %>% select(nswprice,nswdemand,vicprice,vicdemand,class)
...
df2 <-  rbind(results_df,data.frame(Model = "PDD", Mean_Accuracy =rowMeans(as.data.frame(acc_list_pdi), na.rm = TRUE), Drift_Count = drift_number ),
...
df2 <- cbind(df2,Batch = number_of_batch,most_important)
