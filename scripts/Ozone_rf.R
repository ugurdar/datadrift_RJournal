...
df111 <-  rbind(results_df,data.frame(Model = "PDD", Mean_Accuracy =rowMeans(as.data.frame(acc_list_pdi), na.rm = TRUE), Drift_Count = drift_number ),
...
df111$Mean_Accuracy <- format(df111$Mean_Accuracy, scientific = FALSE)
df111 <- cbind(df111,Batch = number_of_batch,most_important)
