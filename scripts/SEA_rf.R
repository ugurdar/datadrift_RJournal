...
rf <- randomForest((class)~ ., data = data_sudden[first_interval,],ntree=50)
rf1 <- rf
...
for(interval_i in seq(max(test_interval),length(data_sudden$class),by=inc_size)){
  ...
  preds_temp <- predict(rf,na.omit(data_sudden[new_interval,]), type = "class")
  acc_temp <- mean(preds_temp==na.omit(data_sudden[new_interval,]$class),na.rm=TRUE )
  # acc_temp <- RMSE(preds_temp,data_sudden[new_interval,]$class, )
  acc_list_pdi[[acc_list_index]] <- acc_temp

  if( length(data_sudden$class)- interval_i < 5){
    break
  }
  ...
  if(pdi_temp >pdi_cutoff*1.0 & l2_temp >l2_cutoff*1.0 & l2_der_temp > l2_der_cutoff*1.0){
    ...
    drift_number_pdd <- drift_number_pdd+1
    ...
    plot_list[[drift_number_pdd+1]] <- p2
    save_plot(experiment_result_path, number_of_batch,
    ...
    data_new <- rbind(
      na.omit(data_sudden[new_interval, ]),
      data_new)
    print(nrow(data_new))
    set.seed(123)
    rf <- randomForest(class ~ ., data = data_new,ntree=50)
    explainer_base <- DALEX::explain(model = rf,
                                     data = data_new,
                                     y =  data_new$class,
                                     verbose=FALSE,
...
pdd_means <- rowMeans(as.data.frame(acc_list_pdi),na.rm=TRUE)
...
df<- rbind(results_df,data.frame(Model = "PDD", Mean_Accuracy =rowMeans(as.data.frame(acc_list_pdi), na.rm = TRUE), Drift_Count = drift_number_pdd),
           ...
           data.frame(Model = "batch sizes", Mean_Accuracy=batch_sizes$train_set,Drift_Count = batch_sizes$test_set))
