...
rf <- glm(as.factor(target)~ ., data = data_sudden[first_interval,],family="binomial")
rf1 <- rf
...
test_accuracy <- mean(test_preds==data_sudden[test_interval,]$target )
print(test_accuracy)
train_accuracy <- mean(train_preds==data_sudden[first_interval,]$target )
...
for(interval_i in seq(max(test_interval),length(data_sudden$target),by=inc_size)){
  ...
  preds_temp <- ifelse(preds_temp>0.5,1,0)
  ...
  if(pdi_temp >pdi_cutoff*1.0 & l2_temp >l2_cutoff*1.0 & l2_der_temp > l2_der_cutoff*1.0){
    ...
    print(nrow(data_new))
    set.seed(123)
    rf <- glm(as.factor(target) ~ ., data = data_new,family="binomial")
    explainer_base <- DALEX::explain(model = rf,
                                     data = data_new,
                                     y =  data_new$target,
                                     verbose=FALSE,
...
run_drift_detection <- function(model_type, rf1, data_sudden, test_interval, inc_size) {
  ...
  for(interval_i in seq(max(test_interval), length(data_sudden$target), by=inc_size)){
    ...
    acc_list_index <- acc_list_index + 1
    data_stream <- na.omit( 1-(data_sudden[new_interval,]$target == preds_temp))
